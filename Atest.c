...
char*** create_alphabetical_array(char** word_array, int* alpha_count_array){
  ...
  int i,j,l,z=0,k=0,x;
	...
	max_word_length=counter_array[0]; /*Initalized variable with readable name, max_word_length*/
	alphabetical_array = malloc(sizeof(char*) * 27);
	for (i=0;i<27;i++){
		alphabetical_array[i] = malloc(sizeof(char*) * (max_word_length));
		for(j=0;j<max_word_length;j++){
			alphabetical_array[i][j] = malloc(sizeof(char*) * 45); /*The first part of the 3d array is 26 for each array of arrays for each letter*/
		}
	}
...
for (i=0;i<26;i++){
  ...
  if (alphabetical_array[i][0][0]=='\0'){
		for (x=0;x<max_word_length;x++){
			alphabetical_array[i][0]=NULL;
...
return(0);
*/
printf("-%s-",alphabetical_array[0][25]);
return(alphabetical_array);
}

void free_alphabetical_array(char*** alphabetical_array){
	int i=0,x=0;
	while (alphabetical_array[x][0]!=NULL){
		free(alphabetical_array[x][i]);
		x++;
		while (alphabetical_array[i]!=NULL){
